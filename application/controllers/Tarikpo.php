...
class Tarikpo extends CI_controller{
  
  function __construct(){
    parent::__construct();
     $this->load->model('tarikpomodel','model');
  }
  ...
  function index(){
    ...
    $this->load->view('tarikpo/tarikpoview',$data);
  }
    ...
    if ($pch=='0') {
      for ($i=0; $i<count($data1['datajt']); $i++) { 
        $html = $html."<tr>
                          ...
                          <td>".$data1['datajt'][$i]['Unit_Name']."</td>
  ...
  function simpanpo($kodepo,$noindex){
    ...
    if($data==2){
      ...
      $objResponse->script("alert('Data PO ".$kodepo." Sudah Ada Dan Berhasil Di Update')");    
    }
